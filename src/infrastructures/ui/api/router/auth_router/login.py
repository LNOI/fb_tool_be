...
async def validate_user(
    ...
    if "user_id" in request.path_params:
        ...
        if v is None:
            ...
            user = await user_usecase.get_user(
            ...
            set_cache(
                f"user:{request.path_params.get('user_id')}", data, resp["expires_in"]
            )
        else:
            user_scopes = v["user"]["scopes"]
        if any([scope in security_scopes.scopes for scope in user_scopes]):
            return None
    else:
        response = await validate_access_token(token)
        if response.status_code != 200:
...
async def first_login(
    user: CreateUserRequestDto, current_user: Dict = Depends(get_current_user)
):
    ...
    data = {"access_token": current_user["access_token"], "user": {"scopes": []}}
    if user_exists:
        data["user"]["scopes"] = user_exists.scopes
        set_cache(f"user:{user.id}", data, ex=current_user["expires_in"])
        return CustomJSONResponse(
            status_code=status.HTTP_200_OK, message="User already exists"
        )
