...
async def list_post(page:int = 1,page_size : int = 25):
    query = select(PostModel).where(PostModel.deleted_at.is_(None)).limit(page_size).offset((page-1)*page_size).order_by(PostModel.created_at.desc())
